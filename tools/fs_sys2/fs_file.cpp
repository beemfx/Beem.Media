...
fs_dword CLFile::Read(void* pOutBuffer, fs_dword nSize)
{
	...
	{
		//Doing a memory read...
		FS_ErrPrintA("Performing memory read...", ERR_LEVEL_SUPERDETAIL);
	...
	{
		//Doing a disk read...
		FS_ErrPrintA("Performing disk read...", ERR_LEVEL_SUPERDETAIL);
	...
	//FS_ErrPrintW(L"Read %d bytes.", ERR_LEVEL_ERROR, nSize);
	return nSize;
}
...
fs_dword CLFile::Seek(LF_SEEK_TYPE nMode, fs_long nOffset)
{
	...
	FS_ErrPrintA("Seeked to %d out of %d", ERR_LEVEL_SUPERDETAIL, m_nFilePointer, m_nSize);
