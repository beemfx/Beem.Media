...
void CLFileSystem::PrintMountInfo(FS_PRINT_MOUNT_INFO_TYPE Type)
{
	...
	{
		FS_ErrPrintW(L"Base mounted at \"%s\"", ERR_LEVEL_ALWAYS, this->m_szBasePath);
		FS_ErrPrintW(L"Mounted Files:", ERR_LEVEL_ALWAYS);
	}
...
CLFileSystem::MOUNT_FILE* CLFileSystem::GetFileInfoW( fs_cstr16 szMountPath , CPartTable** OwnerTable )
{
	fs_pathW sFixed;
	FS_FixCaseW(sFixed, szMountPath);
...
CLFileSystem::MOUNT_FILE* CLFileSystem::GetFileInfoA( fs_cstr8 szMountPath , CPartTable** OwnerTable )
{
	fs_pathW szMountedFileW;
	mbstowcs(szMountedFileW, szMountPath, FS_MAX_PATH);
	return GetFileInfoW( szMountedFileW , OwnerTable );
}
...
fs_dword CLFileSystem::MountBaseW(fs_cstr16 szOSPath)
{
	if(m_bBaseMounted)
	{
		FS_ErrPrintW(L"MountBase Error 1: The base file system is already mounted.  Call UnMountAll before mounting the base.", ERR_LEVEL_ERROR);
		return 0;
	}
	
	if(!BDIO_ChangeDirW(szOSPath))
	{
		FS_ErrPrintW(L"MountBase Error 2: Could not mount base file system.", ERR_LEVEL_ERROR);
		return 0;
	}
	
	BDIO_GetCurrentDirW(m_szBasePath, FS_MAX_PATH);	
	m_bBaseMounted=FS_TRUE;
	fs_dword nMountCount=MountW(L".", L"/", 0);
	FS_ErrPrintW(L"Mounted \"%s\" to \"/\"", ERR_LEVEL_DETAIL, m_szBasePath);
	return nMountCount;
}

fs_dword CLFileSystem::MountBaseA(fs_cstr8 szOSPath)
{
	if(m_bBaseMounted)
	{
		FS_ErrPrintW(L"MountBase Error 3: The base file system is already mounted.  Call UnMountAll before mounting the base.", ERR_LEVEL_ERROR);
		return 0;
	}
	
	if(!BDIO_ChangeDirA(szOSPath))
	{
		FS_ErrPrintW(L"MountBase Error 4: Could not mount base file system.", ERR_LEVEL_ERROR);
		return 0;
	}
	
	fs_pathA szBasePath;
	BDIO_GetCurrentDirA(szBasePath, FS_MAX_PATH);
	mbstowcs(m_szBasePath, szBasePath, FS_MAX_PATH);
		
	m_bBaseMounted=FS_TRUE;
	fs_dword nMountCount=MountA(".", "/", 0);
	FS_ErrPrintW(L"Mounted \"%s\" to \"/\"", ERR_LEVEL_DETAIL, m_szBasePath);
	return nMountCount;
}
...
fs_dword CLFileSystem::MountLPKW(fs_cstr16 szMountedFileOrg, fs_dword Flags)
{
	fs_pathW szMountedFile;
	FS_FixCaseW(szMountedFile, szMountedFileOrg);

	MOUNT_FILE* pMountFile=GetFileInfoW(szMountedFile,FS_NULL);
	if(!pMountFile)
	{
		FS_ErrPrintW(L"MountLPK Error 5: The archive file \"%s\" has not been mounted.", ERR_LEVEL_ERROR, szMountedFile);
		return 0;
	}
	fs_pathW szMountDir;
	FS_GetDirFromPathW(szMountDir, szMountedFile);
	FS_ErrPrintW(L"Expanding \"%s\" to \"%s\"", ERR_LEVEL_NOTICE, szMountedFile, szMountDir);
	
	CLArchive lpkFile;
	if(!lpkFile.OpenW(pMountFile->szOSFileW))
	{
		FS_ErrPrintW(L"Could not mount \"%s\".  Possibly not a valid archive.", ERR_LEVEL_ERROR, szMountedFile);
		return 0;
	}
	...
	for(fs_dword i=0; i<lpkFile.GetNumFiles(); i++)
	{
		...
		swprintf(mountFile.szMountFileW, FS_MAX_PATH, L"%s%s", szMountDir, fileInfo.szFilename);
...
fs_dword CLFileSystem::MountLPKA(fs_cstr8 szMountedFileOrg, fs_dword Flags)
{
	fs_pathA szMountedFile;
	FS_FixCaseA(szMountedFile, szMountedFileOrg);

	fs_pathW szMountedFileW;
	mbstowcs(szMountedFileW, szMountedFile, FS_MAX_PATH);
	MOUNT_FILE* pMountFile=GetFileInfoW( szMountedFileW , FS_NULL );
	if(!pMountFile)
	{
		FS_ErrPrintW(L"MountLPK Error 7: The archive file \"%s\" has not been mounted.", ERR_LEVEL_ERROR, szMountedFileW);
		return 0;
	}
	fs_pathW szMountDir;
	FS_GetDirFromPathW(szMountDir, szMountedFileW);
	FS_ErrPrintW(L"Expanding \"%s\" to \"%s\"", ERR_LEVEL_NOTICE, szMountedFileW, szMountDir);
	
	fs_pathA szOSFileA;
	wcstombs(szOSFileA, pMountFile->szOSFileW, FS_MAX_PATH);
...
fs_dword CLFileSystem::MountDirW(fs_cstr16 szOSPath, fs_cstr16 szMount, fs_dword Flags)
{
	fs_dword nMountCount=0;
	fs_bool bRes=0;
	
	fs_pathW szSearchPath;
	...
	if(!bRes)
	{
		FS_ErrPrintW(L"Mount Error 10: Could not mount \"%s\".", ERR_LEVEL_ERROR, mountInfo.szOSFileW);
	...
	do
	{
		...
		FS_FixCaseW(sMountFile.szMountFileW, sMountFile.szMountFileW);
		...
		else
		{
			...
			{
				FS_ErrPrintW(L"\"%s\" is too large to mount!", ERR_LEVEL_ERROR, sMountFile.szOSFileW);
				continue;
			}
			...
			if(MountFile(&sMountFile, Flags))
				...
				FS_ErrPrintW(L"Mount Error 11: Could not mount \"%s\".", ERR_LEVEL_ERROR, mountInfo.szOSFileW);
		}
...
fs_dword CLFileSystem::MountDirA(fs_cstr8 szOSPath, fs_cstr8 szMount, fs_dword Flags)
{
	fs_dword nMountCount=0;
	fs_bool bRes=0;
	
	fs_pathA szSearchPath;
	_snprintf(szSearchPath, FS_MAX_PATH, "%s\\*", szOSPath);
	//Recursivley mount all files...
	...
	{
		FS_ErrPrintA(
			"Mount Error 12: Could not mount any files.  \"%s\" may not exist.",
			ERR_LEVEL_ERROR,
			szOSPath);
		return 0;
	}
	...
	mountInfo.nCmpSize=mountInfo.nOffset=mountInfo.nSize=0;
	mountInfo.Flags=MOUNT_FILE::MOUNT_FILE_DIRECTORY;
	mbstowcs(mountInfo.szOSFileW, szOSPath, FS_MAX_PATH);
	mbstowcs(mountInfo.szMountFileW, szMount, FS_MAX_PATH);
	//Directories take the same overwrite priority, so overwritten
	//directories will be used when creating new files.
	bRes=MountFile(&mountInfo, Flags);
	if(!bRes)
	{
		FS_ErrPrintW(L"Mount Error 13: Could not mount \"%s\".", ERR_LEVEL_ERROR, mountInfo.szOSFileW);
		BDIO_FindClose(hFind);
		return nMountCount;
	}
	...
	do
	{
		//Generate the mounted path and OS path..
		MOUNT_FILE sMountFile;
		fs_pathA szOSFileA, szMountFileA;
		_snprintf(szOSFileA, FS_MAX_PATH, "%s\\%s", szOSPath, sData.szFilename);
		_snprintf(szMountFileA, FS_MAX_PATH, "%s%s", szMount, sData.szFilename);
		
		mbstowcs(sMountFile.szOSFileW, szOSFileA, FS_MAX_PATH);
		mbstowcs(sMountFile.szMountFileW, szMountFileA, FS_MAX_PATH);
		FS_FixCaseW(sMountFile.szMountFileW, sMountFile.szMountFileW);
		...
		if(sData.bDirectory)
		{
			...
			if(FS_CheckFlag(Flags, MOUNT_MOUNT_SUBDIRS))
			{
				strncat(szMountFileA, "/", FS_min<fs_long>(1, FS_MAX_PATH-strlen(szMountFileA)));
		...
		else
		{
			...
			if(MountFile(&sMountFile, Flags))
				...
				FS_ErrPrintW(L"Mount Error 14: Could not mount \"%s\".", ERR_LEVEL_ERROR, mountInfo.szOSFileW);
		}
...
fs_dword CLFileSystem::MountW(fs_cstr16 szOSPathToMount, fs_cstr16 szMountToPathOrg, fs_dword Flags)
{
	fs_pathW szMountToPath;
	FS_FixCaseW(szMountToPath, szMountToPathOrg);
	...
	FS_ErrPrintW(L"Mounting \"%s\" to \"%s\"...", ERR_LEVEL_NOTICE, szOSPath, szMount);
	
	return MountDirW(szOSPath, szMount, Flags);
}

fs_dword CLFileSystem::MountA(fs_cstr8 szOSPathToMount, fs_cstr8 szMountToPathOrg,	fs_dword Flags)
{
	fs_pathA szMountToPath;
	FS_FixCaseA(szMountToPath, szMountToPathOrg);
	...
	if(szMountToPath[strlen(szMountToPath)-1]!=L'/')
		_snprintf(szMount, FS_MAX_PATH, "%s/", szMountToPath);
	else
		_snprintf(szMount, FS_MAX_PATH, "%s", szMountToPath);
		
	fs_pathW szOSPathW, szMountW;
	mbstowcs(szOSPathW, szOSPath, FS_MAX_PATH);
	mbstowcs(szMountW, szMount, FS_MAX_PATH);
	FS_ErrPrintW(L"Mounting \"%s\" to \"%s\"...", ERR_LEVEL_NOTICE, szOSPathW, szMountW);
	
	return MountDirA(szOSPath, szMount, Flags);
}

fs_bool CLFileSystem::ExistsA(fs_cstr8 szFilename)
{
	//Case is fixed by GetFileInfoA
	const MOUNT_FILE* pMountInfo=GetFileInfoA( szFilename , FS_NULL );
	return pMountInfo!=FS_NULL;
}

fs_bool CLFileSystem::ExistsW(fs_cstr16 szFilename)
{
	//Case is fixed by GetFileInfoW
	const MOUNT_FILE* pMountInfo=GetFileInfoW( szFilename , FS_NULL );
	return pMountInfo!=FS_NULL;
}

fs_bool CLFileSystem::UnMountAll()
{
	FS_ErrPrintW(L"Clearing partition table...", ERR_LEVEL_NOTICE);
	...
	FS_ErrPrintW(L"Unmounting the base...", ERR_LEVEL_NOTICE);
	m_bBaseMounted=FS_FALSE;
	m_szBasePath[0]=0;
	return FS_TRUE;
}
...
fs_bool CLFileSystem::MountFile(MOUNT_FILE* pFile, fs_dword dwFlags)
{
	...
	#endif
	FS_ErrPrintW(L"Mounting \"%s\"", ERR_LEVEL_SUPERDETAIL, pFile->szMountFileW);
...
*********************************************************/
CLFile* CLFileSystem::OpenFileW(fs_cstr16 szFilename, fs_dword Access, fs_dword CreateMode)
{
	...
	{
		FS_ErrPrintW(L"OpenFile Error: Cannot open a file with memory access and write access.", ERR_LEVEL_ERROR);
		return FS_NULL;
	}
	...
	if(CreateMode==LF_OPEN_EXISTING)
	{
		if(!pMountInfo)
		{
			FS_ErrPrintW(L"OpenFile Warning: \"%s\" could not be found in the file system.", ERR_LEVEL_WARNING, szFilename);
			return FS_NULL;
		}
	...
	else if(CreateMode==LF_CREATE_NEW)
	{
		if(pMountInfo)
		{
			FS_ErrPrintW(L"OpenFile Error: Cannot create \"%s\", it already exists.", ERR_LEVEL_ERROR, szFilename);
			return FS_NULL;
		}
	...
	{
		FS_ErrPrintW(L"OpenFile Error: Invalid creation mode specifed.", ERR_LEVEL_ERROR);
		return FS_NULL;
	}
	...
	if(bOpenExisting)
	{
		...
		{
			FS_ErrPrintW(L"OpenFile Error: \"%s\" is a directory, not a file.", ERR_LEVEL_ERROR, pMountInfo->szMountFileW);
			return FS_NULL;
		}
		...
		{
			FS_ErrPrintW(L"OpenFile Error: \"%s\" cannot be created, because an existing file is read only.",
				ERR_LEVEL_ERROR,
				szFilename);
		
			return FS_NULL;
		}
		BDIO_FILE bdioFile=BDIO_OpenW(pMountInfo->szOSFileW, bNew?LF_CREATE_ALWAYS:LF_OPEN_EXISTING, Access);
		if(!bdioFile)
		{
			FS_ErrPrintW(L"Could not open the OS file: \"%s\".", ERR_LEVEL_ERROR, pMountInfo->szOSFileW);
			return FS_NULL;
		}
	...
	else
	{
		...
		fs_pathW szFile;
		FS_GetFileNameFromPathW(szFile, szFilename);
		FS_FixCaseW(szFile, szFile);
		...
		//Create the OS file.
		BDIO_FILE bdioFile=BDIO_OpenW(mountInfo.szOSFileW, LF_CREATE_NEW, Access);
		if(!bdioFile)
		{
			FS_ErrPrintW(
...
CLFile* CLFileSystem::OpenFileA(fs_cstr8 szFilename, fs_dword Access, fs_dword CreateMode)
{
	fs_bool bOpenExisting=FS_TRUE;
	fs_bool bNew=FS_FALSE;
	
	//fs_pathW szFilenameW;
	//mbstowcs(szFilenameW, szFilename, FS_MAX_PATH);
	
	//Check the flags to make sure they are compatible.
	if(FS_CheckFlag(Access, LF_ACCESS_WRITE) && FS_CheckFlag(Access, LF_ACCESS_MEMORY))
	{
		FS_ErrPrintA("OpenFile Error: Cannot open a file with memory access and write access.", ERR_LEVEL_ERROR);
		return FS_NULL;
	}
	//Note that if creating a new file, then we shouldn't end here.
	const MOUNT_FILE* pMountInfo=GetFileInfoA(szFilename,FS_NULL);
	...
	if(CreateMode==LF_OPEN_EXISTING)
	{
		if(!pMountInfo)
		{
			FS_ErrPrintA("OpenFile Warning: \"%s\" could not be found in the file system.", ERR_LEVEL_WARNING, szFilename);
			return FS_NULL;
		}
		bOpenExisting=FS_TRUE;
		bNew=FS_FALSE;
	...
	else if(CreateMode==LF_CREATE_NEW)
	{
		if(pMountInfo)
		{
			FS_ErrPrintA("OpenFile Error: Cannot create \"%s\", it already exists.", ERR_LEVEL_ERROR, szFilename);
			return FS_NULL;
		}
		bOpenExisting=FS_FALSE;
		bNew=FS_TRUE;
	...
	{
		FS_ErrPrintA("OpenFile Error: Invalid creation mode specifed.", ERR_LEVEL_ERROR);
		return FS_NULL;
	}
	...
	if(bOpenExisting)
	{
		...
		if(FS_CheckFlag(pMountInfo->Flags, MOUNT_FILE::MOUNT_FILE_READONLY) && bNew)
		{
			FS_ErrPrintA("OpenFile Error: \"%s\" cannot be created, because an existing file is read only.",
				ERR_LEVEL_ERROR,
				szFilename);
		...
		fs_pathA szOSFileA;
		wcstombs(szOSFileA, pMountInfo->szOSFileW, FS_MAX_PATH);
		BDIO_FILE bdioFile=BDIO_OpenA(szOSFileA, bNew?LF_CREATE_ALWAYS:LF_OPEN_EXISTING, Access);
		if(!bdioFile)
		{
			FS_ErrPrintW(L"Could not open the OS file: \"%s\".", ERR_LEVEL_ERROR, pMountInfo->szOSFileW);
			return FS_NULL;
	...
	else
	{
		...
		//the partition table.
		fs_pathA szDir;
		FS_GetDirFromPathA(szDir, szFilename);
		//The mount filename will be the same
		mbstowcs(mountInfo.szMountFileW, szFilename, FS_MAX_PATH);
		const MOUNT_FILE* pDirInfo=GetFileInfoA(szDir,FS_NULL);
		if(!pDirInfo || !FS_CheckFlag(pDirInfo->Flags, MOUNT_FILE::MOUNT_FILE_DIRECTORY))
		{
			FS_ErrPrintA(
		...
		fs_pathA szFile;
		fs_pathW szFileW;
		FS_GetFileNameFromPathA(szFile, szFilename);
		mbstowcs(szFileW, szFile, FS_MAX_PATH);
		FS_FixCaseW(szFileW, szFileW);
		//The OS filename is the information from the dir + the filename
		//Note that the OS separator must be inserted.
		_snwprintf(mountInfo.szOSFileW, FS_MAX_PATH, L"%s%c%s", pDirInfo->szOSFileW, BDIO_GetOsPathSeparator(), szFileW);
		...
		//Create the OS file.
		fs_pathA szOSFileA;
		wcstombs(szOSFileA, mountInfo.szOSFileW, FS_MAX_PATH);
		BDIO_FILE bdioFile=BDIO_OpenA(szOSFileA, LF_CREATE_NEW, Access);
		if(!bdioFile)
		{
			FS_ErrPrintW(
				L"OpenFile Error: Could not create new file: \"%s\".",
...
CLFile* CLFileSystem::OpenExistingFile(const MOUNT_FILE* pMountInfo, BDIO_FILE File, fs_dword Access, fs_bool bNew)
{
	...
	if(!pFile)
	{
		FS_ErrPrintW(L"OpenFile Error: Could not allocate memory for file.", ERR_LEVEL_ERROR);
		return FS_NULL;
	}
	...
	{
		FS_ErrPrintW(L"OpenFile Error: \"%s\" is read only, cannot open for writing.", ERR_LEVEL_ERROR, pFile->m_szPathW);
		BDIO_Close(pFile->m_BaseFile);
		delete pFile;
		return FS_NULL;
	}
	
	FS_ErrPrintW(L"Opening \"%s\"...", ERR_LEVEL_DETAIL, pFile->m_szPathW);
	...
	if(FS_CheckFlag(pFile->m_nAccess, LF_ACCESS_MEMORY))
	{
		...
		if(!pFile->m_pData)
		{
			FS_ErrPrintW(L"OpenFile Error: Could not allocte memory for \"%s\".", ERR_LEVEL_ERROR, pFile->m_szPathW);
			BDIO_Close(pFile->m_BaseFile);
			delete pFile;
			return FS_NULL;
		}
...
fs_bool CLFileSystem::CloseFile(CLFile* pFile)
{
	...
	if(!pFile)
		return FS_FALSE;
	FS_ErrPrintW(L"Closing \"%s\"...", ERR_LEVEL_DETAIL, pFile->m_szPathW);
	if(pFile->m_BaseFile)
	{
	...
	if(FS_CheckFlag(pFile->m_nAccess, LF_ACCESS_WRITE))
	{
		...
		const MOUNT_FILE* pMountInfo=GetFileInfoW(pFile->m_szPathW,&OwnerTable);
		if(!pMountInfo)
		{
			FS_ErrPrintW(L"CloseFile Error: Could not update partition table.", ERR_LEVEL_ERROR);
		}
		else
		{
...
void CLFileSystem::EnumerateFilesOfTypeA( fs_cstr8 Ext , FS_ENUMERATE_FUNCA EnumerateFunc , void* Data )
{
	for( CPartTable* Table = m_PrtTableList; FS_NULL != Table; Table = Table->GetNext() )
	{
		Table->EnumerateFilesOfTypeA( Ext , EnumerateFunc , Data );
	}
}

void CLFileSystem::EnumerateFilesOfTypeW( fs_cstr16 Ext , FS_ENUMERATE_FUNCW EnumerateFunc , void* Data )
{
	for( CPartTable* Table = m_PrtTableList; FS_NULL != Table; Table = Table->GetNext() )
	{
		Table->EnumerateFilesOfTypeW( Ext , EnumerateFunc , Data );
	}
}
...
CLFileSystem::MOUNT_FILE* CLFileSystem::CPartTable::FindFile(fs_cstr16 szFile)
{
		...
		for(MOUNT_FILE_EX* pItem=m_HashList[nHash]; pItem; pItem=pItem->pNext)
		{
			if(wcscmp(pItem->szMountFileW, pFile->szMountFileW)==0)
			{
				...
				if(FS_CheckFlag(Flags, MOUNT_FILE_OVERWRITELPKONLY))
				{
					...
					{
						FS_ErrPrintW(L"Copying %s over old archive file.", ERR_LEVEL_DETAIL, pFile->szMountFileW);
						memcpy(pItem, pFile, sizeof(MOUNT_FILE));
						return FS_TRUE;
					}
					else
					{
						FS_ErrPrintW(L"Cannot mount %s.  A file with the same name has already been mounted.", ERR_LEVEL_DETAIL, pFile->szMountFileW);
						return FS_FALSE;
					}	
				}
				else if(FS_CheckFlag(Flags, MOUNT_FILE_OVERWRITE))
				{
					FS_ErrPrintW(L"Copying %s over old file.", ERR_LEVEL_DETAIL, pFile->szMountFileW);
					memcpy(pItem, pFile, sizeof(MOUNT_FILE));
					return FS_TRUE;
				}
				else
...
fs_dword CLFileSystem::CPartTable::GetHashValue1024(fs_cstr16 szString)
{
...
void CLFileSystem::CPartTable::PrintMountInfo(FS_PRINT_MOUNT_INFO_TYPE Type)
{
	...
	if( PRINT_MOUNT_INFO_FULL == Type )
	{
		FS_ErrPrintA("FLAGS  SIZE       PACKED    PATH", ERR_LEVEL_ALWAYS);
		FS_ErrPrintA("-----  ----       ------    ----", ERR_LEVEL_ALWAYS);
	}
	fs_dword nFiles=0;
	for(fs_dword i=0; i<HASH_SIZE; i++)
	{
		for(MOUNT_FILE_EX* pItem=m_HashList[i]; pItem; pItem=pItem->pNext)
		{
			...
			if( PRINT_MOUNT_INFO_FILENAME ==Type )
			{
					...
					FS_ErrPrintW(szOutput, ERR_LEVEL_ALWAYS);
			}
			else
			{
				PrintFileInfo(pItem);
	...
	if( PRINT_MOUNT_INFO_FULL == Type )
	{
		FS_ErrPrintA("Totals: %d files", ERR_LEVEL_ALWAYS, nFiles);
	}
}
...
void CLFileSystem::CPartTable::PrintFileInfo(MOUNT_FILE_EX* pFile)
{
	...
	fs_char16 szOutput[29+FS_MAX_PATH];
	szOutput[5]=' ';
	...
	FS_ErrPrintW(szOutput, ERR_LEVEL_DETAIL);
}

void CLFileSystem::CPartTable::EnumerateFilesOfTypeA( fs_cstr8 _Ext , FS_ENUMERATE_FUNCA EnumerateFunc , void* Data )
{
	fs_pathW Ext;
	mbstowcs(Ext, _Ext, FS_MAX_PATH);

	for(fs_dword HashPos=0; HashPos<HASH_SIZE; HashPos++)
	{
		for(MOUNT_FILE_EX* pItem=m_HashList[HashPos]; pItem; pItem=pItem->pNext)
		{	
			fs_cstr16 FileExt = &pItem->szMountFileW[pItem->ExtOffset];
			if( 0 == _wcsicmp( FileExt , Ext ) )
			{
				fs_pathA FilenameA;
				wcstombs( FilenameA , pItem->szMountFileW , FS_MAX_PATH );
				EnumerateFunc( FilenameA , Data );
			}
		}
	}
}

void CLFileSystem::CPartTable::EnumerateFilesOfTypeW( fs_cstr16 Ext , FS_ENUMERATE_FUNCW EnumerateFunc , void* Data )
{
	for(fs_dword HashPos=0; HashPos<HASH_SIZE; HashPos++)
	{
		for(MOUNT_FILE_EX* pItem=m_HashList[HashPos]; pItem; pItem=pItem->pNext)
