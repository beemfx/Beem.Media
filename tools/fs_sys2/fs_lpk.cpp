...
fs_bool CLArchive::CreateNewW(fs_cstr16 szFilename)
{
	Close();
	
	m_File=BDIO_OpenW(szFilename, LF_CREATE_ALWAYS, LF_ACCESS_READ|LF_ACCESS_WRITE);
	if(!m_File)
		return FS_FALSE;
		
	m_bWriteable=FS_TRUE;
	m_TempFile=BDIO_OpenTempFileW(LF_CREATE_ALWAYS, LF_ACCESS_READ|LF_ACCESS_WRITE|LF_ACCESS_BDIO_TEMP);
	if(!m_TempFile)
	{
		BDIO_Close(m_File);
		m_File=NULL;
...
fs_bool CLArchive::CreateNewA(fs_cstr8 szFilename)
{
	Close();
	
	m_File=BDIO_OpenA(szFilename, LF_CREATE_ALWAYS, LF_ACCESS_READ|LF_ACCESS_WRITE);
	if(!m_File)
		return FS_FALSE;
		
	m_bWriteable=FS_TRUE;
	m_TempFile=BDIO_OpenTempFileA(LF_CREATE_ALWAYS, LF_ACCESS_READ|LF_ACCESS_WRITE|LF_ACCESS_BDIO_TEMP);
	if(!m_TempFile)
	{
		BDIO_Close(m_File);
		m_File=NULL;
...
fs_bool CLArchive::OpenW(fs_cstr16 szFilename, fs_dword Flags)
{
	Close();
	...
	m_File=BDIO_OpenW(szFilename, LF_OPEN_ALWAYS, dwAccess);
	if(!m_File)
	{
		Close();
		return FS_FALSE;
...
fs_bool CLArchive::OpenA(fs_cstr8 szFilename, fs_dword Flags)
{
	Close();
	...
	m_File=BDIO_OpenA(szFilename, LF_OPEN_ALWAYS, dwAccess);
	if(!m_File)
	{
		Close();
		return FS_FALSE;
...
fs_dword CLArchive::DoAddFile(BDIO_FILE fin, fs_cstr16 szNameInArc, fs_dword Flags)
{	
...
fs_dword CLArchive::AddFileW(fs_cstr16 szFilename, fs_cstr16 szNameInArc, fs_dword Flags)
{
	if(!m_bOpen || !m_bWriteable)
		return 0;
		
	BDIO_FILE fin=BDIO_OpenW(szFilename, LF_OPEN_EXISTING, LF_ACCESS_READ);
	if(!fin)
		return 0;
	
	fs_pathW sFinalName;
	FS_FixCaseW(sFinalName, szNameInArc);

	fs_dword nSize=DoAddFile(fin, sFinalName, Flags);
	BDIO_Close(fin);
	return nSize;	
}

fs_dword CLArchive::AddFileA(fs_cstr8 szFilename, fs_cstr8 szNameInArc, fs_dword Flags)
{
	if(!m_bOpen || !m_bWriteable)
		return 0;
		
	BDIO_FILE fin=BDIO_OpenA(szFilename, LF_OPEN_EXISTING, LF_ACCESS_READ);
	if(!fin)
		return 0;
		
	fs_pathW szWideName;
	mbstowcs(szWideName, szNameInArc, FS_MAX_PATH);

	fs_pathW sFinalName;
	FS_FixCaseW(sFinalName, szWideName);
...
fs_dword CLArchive::GetFileRef(const fs_pathW szName)
{
...
fs_byte* CLArchive::ExtractFile(fs_byte* pOut, fs_dword nRef)
{	
	if(nRef>=m_nCount)
		return FS_NULL;
